...
output_file = '/home/ubuntu/notice_append.txt'

# Define the file patterns you want to search for (like LICENSE.txt, etc.)
notice_file_patterns = ['NOTICE.txt']

# Open the output file in append mode
with open(output_file, 'a') as outfile:
    # Write the initial header to the output file (if needed)
    outfile.write("Notice files content:\n\n")

    # Walk through the directory and its subdirectories
    for root, dirs, files in os.walk(search_directory):
        for file in files:
            # Check if the file starts with "NOTICE" (case insensitive)
            if file.upper().startswith("NOTICE"):
                file_path = os.path.join(root, file)
                
                # Print to console in the required format
                print(f"Path of the notice file is: {file_path}")
                print("---------------------------------------------")
                print("content of the notice file is:")
                print("---------------------------------------------")              
                # Read and print the content of the license file
                with open(file_path, 'r') as notice_file:
